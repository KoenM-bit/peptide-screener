...
export function processBindingData(data: HLABindingData): ProcessedBindingData {
  ...
  const bindings = Object.entries(data).map(
    ([allele, value]): PeptideBinding => {
      const numericValue =
...
export function getBindingLevel(
  value: number
): 'Strong' | 'Moderate' | 'Weak' | 'Non-binder' {
...
export function getBindingColor(level: string): {
  bg: string;
  text: string;
  border: string;
} {
