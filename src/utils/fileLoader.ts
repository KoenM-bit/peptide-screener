...
export async function loadPeptideFile(filename: string): Promise<Record<string, PeptideData>> {
  try {
    ...
    for (const path of pathsToTry) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(`Successfully loaded peptide from: ${path}, keys:`, Object.keys(jsonData).join(', '));

          // Check if the data has the correct structure
          if (jsonData[peptideId]) {
            console.log(`Found expected peptide data structure for ${peptideId}`);
            return jsonData; // Return as is, since it's already in {peptideId: data} format
          } else {
            console.log(`Did not find expected structure for ${peptideId}, wrapping data`);
