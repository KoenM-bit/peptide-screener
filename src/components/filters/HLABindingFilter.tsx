...
const BINDING_LEVELS = [
  {
    id: 'Strong',
    label: 'Strong Binders (≤ 50 nM)',
    color: 'bg-green-100 text-green-800',
  },
  {
    id: 'Moderate',
    label: 'Moderate Binders (≤ 500 nM)',
    color: 'bg-blue-100 text-blue-800',
  },
  {
    id: 'Weak',
    label: 'Weak Binders (≤ 5000 nM)',
    color: 'bg-yellow-100 text-yellow-800',
  },
  {
    id: 'Non-binder',
    label: 'Non-binders (> 5000 nM)',
...
export function HLABindingFilter({ value, onChange }: HLABindingFilterProps) {
  ...
  const toggleGroup = (group: string) => {
    ...
    const allGroupSelected = groupAlleles.every(allele =>
      value.alleles.includes(allele)
    );

    if (allGroupSelected) {
      // Remove all alleles from this group
      const newAlleles = value.alleles.filter(
        allele => !groupAlleles.includes(allele)
      );
      onChange({ ...value, alleles: newAlleles });
    } else {
