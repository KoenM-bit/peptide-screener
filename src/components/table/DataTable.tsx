...
export function DataTable({ data, onRowClick, selectedPeptides, onSelectionChange }: DataTableProps) {
  ...
  const columns = [
    ...
    columnHelper.accessor(row => ({ gene: row.Gene, description: row['Gene description'] }), {
      id: 'gene',
            ...
            {table.getRowModel().rows.map(row => (
              ...
              >
                {row.getVisibleCells().map(cell => (
                  <td key={`cell_${cell.id}`} className="px-3 py-2 whitespace-nowrap text-sm text-gray-900">
